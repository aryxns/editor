...
main_app.title('ARYXN text editor')
...
#italic button 
def change_italic():
	text_slant = tk.font.Font(font=text_editor['font'])
	if text_slant.actual()['slant'] == 'roman':
		text_editor.configure(font=(current_font_family, current_font_size, 'italic'))
	if text_slant.actual()['slant'] == 'italic':
...
#color wheel 
def chaneg_font_color():
	color_var = tk.colorchooser.askcolor()
	text_editor.configure(fg=color_var[1])

font_color_btn.configure(command=chaneg_font_color)
...
def open_file(event=None):
	global url
	url = filedialog.askopenfilename(initialdir=os.getcwd(), title='Select Open File', filetypes=(('Text File', '*.txt'), ('All Files', '*.*')))
	try:
		with open(url, 'r') as reader:
			text_editor.delete(1.0, tk.END)
			text_editor.insert(1.0, reader.read())
...
def save_file(event=None):
	...
	try:
		if url:
			...
			with open(url, 'w', encoding='utf-8') as writer:
...
def exit_file(event=None):
	...
	try:
		if text_changed:
			mbox = messagebox.askyesnocancel('Warning', 'Do you want to save the file god ?')
			if mbox is True:
				if url: 
					content = text_editor.get(1.0, tk.END)
...
def find_func(event=None):
	...
	def replace():
		word = find_input.get()
		replace_text = replacc_input.get()
		content = text_editor.get(1.0, tk.END)
		new_content = content.replace(word, replace_text)
		text_editor.delete(1.0, tk.END)
		text_editor.insert(1.0, new_content)

	find_dialogue = tk.TOP()
